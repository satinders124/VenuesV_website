...
export async function removeUserFromVenue({ uid, venue }) {
  ...
  // Robust removal: case-insensitive, trimmed name comparison – fixes duplicate name ghost
  const currentVenues = (Array.isArray(profile.venues) ? profile.venues : profile.venue ? [profile.venue] : [])
    .map(v => String(v || '').trim())
    .filter(Boolean);
  const targetNameLower = String(venue.name || '').trim().toLowerCase();
  const venues = currentVenues.filter((name) => String(name).trim().toLowerCase() !== targetNameLower);
  ...
  // String-safe assignedUids removal – prevents ghost when uid is uuid vs text
  const assignedUids = (venue.assignedUids || []).map(String).filter((memberId) => String(memberId) !== String(uid));
  const { error: venueError } = await supabase
    .from('venues')
    .update({ assignedUids })
    .eq('id', venue.id);
